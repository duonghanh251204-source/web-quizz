...
final class MysqlPlatformRepository implements PlatformRepositoryInterface
{
    ...
    {
        $stmt = $this->connection->query(
            'SELECT q.id, q.created_by, q.title, q.difficulty, q.created_at,
             ...
             GROUP BY q.id, q.created_by, q.title, q.difficulty, q.created_at, d.title, u.name
             ORDER BY q.id DESC'
        );
    ...
    public function listSubmissions(?int $userId = null): array
    {
        if ($userId === null) {
            $stmt = $this->connection->query(
                'SELECT s.id, s.quiz_id, s.user_id, s.score, s.total_questions, s.total_correct, s.created_at,
